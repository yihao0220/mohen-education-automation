...
截至 2026-08-03，系统保留三个直接业务入口，并新增三条不接管生产流程的 DOCX 只读分析旁路：

| 模块 | 入口 | 状态 | 说明 |
|------|------|------|------|
| 题目录入 | `墨痕快刀/main.py` | 稳定可用 | 英语、理科、文科三核 + 学科覆盖层 |
...
源文档和 P1b 产物继续放在仓库外部。运行题目录入、答案清洗或专项工具时，直接选择当前机器上的业务文档或目录；仓库不保存个人业务路径。
...
全批口径仍有一个独立门禁：目录强制使用 `geography` 覆盖层后，37 份 Source ActionPlan 1.1 共 284 个动作，而既有题答处理基线是 298 个 F1/答案块；14 组差异只集中在《课时分层作业22》（15→8）和《课时分层作业24》（13→6）的材料题合并。这不是 DOCX/WPS 坐标问题，也不能用单样本 8/8 验收代替；必须先确认 284 组是否为新的业务口径，并同步核对答案块，才能进入 37 份全批生产。
...
答案清洗脚本在找到对应题目文档时，会同时生成 `_审核清单.md` 和 `_审核状态.json`。答案录入脚本只接受自动检查通过且文件内容未发生变化的 `_已清洗.docx`；缺状态、检查未通过或清洗后又被修改都会被拦截。
...
- 新旧口径边界：上述 298 是既有题答处理基线；Source ActionPlan 1.1 按目录强制 `geography` 后为 284 个动作。两者尚未完成业务确认和答案块同步，不能混称为同一批次已经对齐
