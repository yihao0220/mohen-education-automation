...
| 教训 | **新模板开发完毕必须同步更新 `main.py`**（该规则现为 `CLAUDE.md` 铁律第 3 条；2026-07-26 前写在 `AGENTS.md` 规范第 4 条） |
...
| 修复 | 新增规则：**收到 docx 样本必须主动检查解析续行模式**（现位于 `.claude/skills/mohen-project-rules/references/通用录入规则.md` 的「解析续行识别」；2026-07-26 前写在 `AGENTS.md` 规范第 5 条） |
