...
> 日期：2026-08-20
> 状态：第一、第二部分已完成影子实现与离线验证；第三至第十部分待逐部分确认和实施
...
- 已新增版本化候选对象、逐引用 include/exclude/unknown 角色判断和确定性编译器；
- 已覆盖来源存在性、完整覆盖、非法重叠、题号顺序与显式重置、连续题号合并、低置信度、unknown、富内容歧义及规则/AI冲突；
- 任一 blocker 出现时固定整批 `units=[]`，不会生成部分成功的 `QuestionUnitV2`；
- 已新增旧 `QuestionUnit` 规则候选影子适配、逐引用/媒体归属差异报告和只读 DOCX CLI；
- 影子产物固定 `mode=shadow`、`production_execution_enabled=false`、`keypress_count=0`；
- 未连接真实 AI，未修改旧题块生产入口，未生成 F1 ActionPlan，未连接 WPS 或执行任何快捷键；
- 现有原始节点扫描器未暴露的 table/media/formula 仍不能冒充已覆盖；第二部分测试不证明真实 DOCX 全适配或 WPS 选区正确。

实施证据见 `问题归档/题目录入/2026-08-26-QuestionUnitV2题目理解与确定性编译.md`。
