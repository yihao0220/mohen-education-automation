...
如果显示 `15 passed, 3 skipped`，说明真实业务目录没有找到或路径填错；这不算完整通过。
...
- GitHub 最新功能代码已克隆到独立 Windows 目录；安装时版本为 `18b9b05`；
- Windows 开发与生产依赖已安装，用户确认安装脚本显示“Windows 开发与生产环境已就绪”；
- Mac 离线开发和 Quick Look 预览已经完成；
- Windows 专项离线测试尚需按本手册执行并保存结果；
- 真实 WPS F1 录入和 52/52 P1b 人工门禁仍未完成，不能仅凭安装成功宣称生产验证通过。
